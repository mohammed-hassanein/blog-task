...
<div class="max-w-3xl mx-auto">
    <!-- Back Link -->
    <a href="{{ route('blog.index') }}" class="inline-flex items-center gap-2 text-blue-600 hover:text-blue-700 font-medium mb-10 group bg-blue-50/50 hover:bg-blue-50 px-4 py-2 rounded-xl transition-all">
    ...
    <article class="bg-white rounded-[2.5rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-10 md:p-16 relative overflow-hidden">
        <!-- Abstract Decoration -->
        <div class="absolute top-0 right-0 w-64 h-64 bg-gradient-to-bl from-blue-50 to-transparent rounded-bl-full -z-10 opacity-60"></div>
        ...
        <div class="flex flex-wrap items-center gap-6 md:gap-10 text-slate-600 mb-12 pb-12 border-b border-slate-100">
            <div class="flex items-center gap-4">
                <div class="w-12 h-12 rounded-full bg-gradient-to-tr from-blue-700 to-blue-500 flex items-center justify-center text-white font-bold text-lg shadow-sm ring-4 ring-blue-50">
                    {{ substr($post->user->name ?? 'A', 0, 1) }}
                </div>
                <div>
                    <p class="text-xs font-bold text-blue-500 uppercase tracking-widest mb-0.5">Author</p>
        ...
        <!-- Post Content -->
        <div class="prose prose-lg md:prose-xl max-w-none prose-slate prose-headings:font-bold prose-headings:tracking-tight prose-a:text-blue-600 hover:prose-a:text-blue-500 prose-img:rounded-2xl prose-img:shadow-lg leading-relaxed font-light">
